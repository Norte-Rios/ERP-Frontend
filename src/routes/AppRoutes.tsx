...
import SetPasswordPage from '../auth/SetPasswordPage.tsx';
// Layouts
...
const AppRoutes = (props: AppRoutesProps) => {
  ...
  return (
    <Routes>
      ...
      <Route element={<ProtectedRoute><Outlet context={props} /></ProtectedRoute>}>
        {/* ROTA DE PERFIL */}
        <Route
          path="/profile"
          element={
            userRole === 'admin' || userRole === 'admin master' ? <MainLayout /> :
            userRole === 'operational' ? <OperationalLayout /> :
        ...
        <Route element={<Outlet context={props} />}>
          ...
          {user && userRole === 'consultant' && currentConsultant && (
            <Route path="/consultant" element={<ConsultantLayout />}>
              <Route path="dashboard" element={<ConsultantDashboardPage consultantName={currentConsultant.fullName} tasks={Object.values(consultantTaskBoard.tasks)} services={consultantServices} payments={[]} taskBoard={consultantTaskBoard} />} />
              ...
              <Route path="tasks" element={<ConsultantTasksPage consultantTaskBoard={consultantTaskBoard} currentConsultant={currentConsultant} onUpdateTaskBoard={props.onUpdateTaskBoard} onUpdateTask={props.onUpdateTask} onDeleteTask={props.onDeleteTask} onAddTaskComment={props.onAddTaskComment} />} />
              <Route path="documents" element={<ConsultantDocumentsPage documents={consultantDocuments} consultantName={currentConsultant.fullName} onAddDocument={(doc) => props.onAddDocument(doc, currentConsultant.fullName)} />} />
              <Route path="documents/:documentId" element={<ConsultantDocumentDetailPage documents={consultantDocuments} onAddComment={(docId, text) => props.onAddCommentToDocument(docId, text, currentConsultant.fullName)} consultantName={currentConsultant.fullName} />} />
              <Route path="logbook" element={<ConsultantLogbookPage announcements={props.announcements} logEntries={props.logEntries} currentConsultant={currentConsultant} onAddEntry={(text) => consultantAuthor && props.onAddLogEntry(text, consultantAuthor)} onAddComment={(entryId, text) => consultantAuthor && props.onAddLogComment(entryId, text, consultantAuthor)} />} />
              <Route path="payments" element={<ConsultantPaymentsPage payments={[]} />} />
            </Route>
          )}
          ...
          {user && userRole === 'client' && currentClient && (
            <Route path="/client" element={<ClientLayout client={currentClient} />}>
              <Route path="dashboard" element={<ClientDashboardPage clientName={currentClient.companyName} services={clientServices} contracts={clientContracts} revenues={clientRevenues} />} />
              <Route path="services" element={<ClientServicesPage services={clientServices} allServices={props.services} currentClient={currentClient} onAddService={props.onAddService} />} />
              <Route path="services/:serviceId" element={<ClientServiceDetailPage services={clientServices} />} />
              <Route path="documents" element={<ClientDocumentsPage documents={clientDocuments} currentClient={currentClient} onAddDocument={(doc) => clientAuthor && props.onAddDocument(doc, clientAuthor.name)} />} />
              <Route path="documents/:documentId" element={<ClientDocumentDetailPage documents={clientDocuments} onAddComment={(docId, text) => clientAuthor && props.onAddCommentToDocument(docId, text, clientAuthor.name)} />} />
              <Route path="financial" element={<ClientFinancialPage contracts={clientContracts} revenues={clientRevenues} />} />
            </Route>
          )}

          {/* ROTAS OPERACIONAIS – SEM PROP user */}
          {user && userRole === 'operational' && (
            <Route path="/operational" element={<OperationalLayout />}>
              ...
              <Route
                ...
                element={
                  <OperationalTasksPage
                    currentUser={{
                      ...user,
                      fullName: operationalUser?.nome || user.email,
                    }}
                  />
                }
              />
              ...
              <Route path="agenda" element={<AgendaPage services={props.services} />} />
              <Route path="meet" element={<OperationalMeetPage />} />
            </Route>
          )}
        </Route>
      </Route>
