...
interface ClientDocumentsPageProps {
  ...
  onAddDocument: (document: Omit<Document, 'id' | 'sentAt' | 'history' | 'comments' | 'status' | 'sender'>) => void;
}
...
const ClientDocumentsPage: React.FC<ClientDocumentsPageProps> = ({ documents, onAddDocument }) => {
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    if (file) {
      ...
      const newDoc = {
        ...
        recipient: { id: 'ADMIN', name: 'Admin', type: 'Consultor' as const }, // Sempre envia para o Admin
      };
    ...
    <div className="container mx-auto">
      ...
      <div className="flex justify-between items-center mb-4">
         <div className="flex border-b">
            ...
            <button onClick={() => setActiveTab('sent')} className={`px-4 py-2 text-sm font-medium ${activeTab === 'sent' ? 'border-b-2 border-indigo-500 text-indigo-600' : 'text-gray-500 hover:text-gray-700'}`}>Enviados por mim</button>
      ...
      {isUploading && (
        ...
        >
          ...
          >
            ...
            <p className="text-sm text-gray-500 mb-4">O destinatário será a equipe de administração da consultoria.</p>
            <div 
            ...
            >
              ...
              <p className="text-xs text-gray-500">ou clique para selecionar (simulado)</p>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-6">
        {filteredDocuments.map((doc) => (
          <Link to={`/client/documents/${doc.id}`} key={doc.id} className="block">
       ...
       {filteredDocuments.length === 0 && (
          <div className="text-center py-12 text-gray-500">
            ...
            <p className="text-sm">Tente ajustar a sua busca ou filtros.</p>
          </div>
        )}
    </div>
  );
