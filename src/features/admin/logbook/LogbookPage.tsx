...
import { LogEntry, Announcement, LogComment } from './types';
...
// Componente para um único anúncio
...
// Componente para uma única entrada do diário
const LogEntryCard = ({ entry, onAddComment }: { entry: LogEntry, onAddComment: (entryId: string, text: string) => void }) => {
...
const LogbookPage: React.FC<LogbookPageProps> = ({ announcements, logEntries, onAddEntry, onAddComment }) => {
        ...
        <div className="container mx-auto max-w-4xl">
            ...
            <div className="space-y-6">
                {logEntries.map(entry => (
                    <LogEntryCard key={entry.id} entry={entry} onAddComment={onAddComment} />
                ))}
            </div>
        </div>
    );
