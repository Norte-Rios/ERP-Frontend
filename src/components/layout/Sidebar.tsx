...
// ATUALIZADO: Importa NavLink e useNavigate
...
const Sidebar = () => {
  // ATUALIZADO: Obtém 'user' e 'logout' do contexto
  const { user, logout } = useAuth();
  const [openMenu, setOpenMenu] = useState<string | null>(null);
  // ATUALIZADO: Inicializa o hook de navegação
  const navigate = useNavigate();

  // --- ATUALIZAÇÕES PARA O AVATAR ---
  ...
  // ATUALIZADO: Função para lidar com o logout
  const handleLogout = () => {
    if (logout) {
  ...
  return (
    <aside className="w-64 bg-brand-green-dark text-white flex flex-col min-h-screen">
      ...
      <div className="px-4 py-4 border-t border-brand-green-light/30 mt-auto">
        ...
        {/* ATUALIZADO: Botão "Sair" agora usa o handleLogout */}
        <button
